...
BMI quantifies the amount of tissue mass (muscle, fat, and bone) in an individual, and then categorize that person as underweight, normal weight, overweight, or obese based on that value. BMI is given by the following equation for weight in pounds and height in inches:

$$BMI = 703\times\frac{weight}{height^{2}}$$

The original dataset is filtered for weights reported in pounds (i.e. 50 $\leq$ weight2 $\leq$ 999) and heights reported in ft/in (i.e. 200 $\leq$ height3 $\leq$ 711). The height column is further converted into values purely in inches. Then BMI values according to the above formula are computed.
...
Classification based on BMI values into BMI classes (i.e. Underweight, Normal, Overweight and Obesity).
